...
| # | Property | Status | Evidence |
| - | -------- | ------ | -------- |
| W1 | A depositor cannot redeem before the vault's redemption delay has passed. | postulated | The delay is read back from the access manager and checked against the input (`access.redemptionDelay`), and the lifecycle test waits it out with `vm.warp`, but no pilot test asserts that redeeming *earlier* reverts. |
...
| P4 | A fuse action against a substrate that was never granted reverts. | postulated | Enforced in [`Erc4626SupplyFuse.sol`](../contracts/fuses/erc4626/Erc4626SupplyFuse.sol) (`Erc4626SupplyFuseUnsupportedVault`) and covered for local fixtures, but no pilot test asserts it against the deployed fuse. |
...
2. **P4** — no test that a non-granted substrate is refused by the *deployed* fuse.
