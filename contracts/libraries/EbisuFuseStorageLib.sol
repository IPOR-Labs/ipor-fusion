...
/// @title Ebisu fuses storage library responsible for managing storage fuses in the Plasma Vault
library EbisuFuseStorageLib {
    /**
     ...
     * - Enables efficient position management and lookup
     * - Supports concentrated liquidity position tracking
     * - Mirrors Liquity V2-style position management for Ebisu protocol
     *
     * Storage Layout:
     ...
     *
     * Usage Pattern:
     * - Updated when creating new Ebisu positions
     * - Referenced during position management
     * - Used for position value calculations
     * - Maintains efficient position tracking for Ebisu protocol
     *
     * Integration Points:
     * - Ebisu position management fuses
     * - Position value calculation systems
     * - Balance tracking mechanisms
     * - Ebisu-specific liquidity operations
     *
     * Security Considerations:
     * - Must accurately track all vault positions
     * - Critical for Ebisu liquidity management
     * - Requires careful index management
     * - Essential for position ownership verification
     * - Parallel structure to Liquity V2 position tracking
     */
    bytes32 private constant EBISU_OWNER_IDS =
        0xeb1234567890abcdef1234567890abcdef1234567890abcdef12345678900000;
    ...
    struct EbisuOwnerIds {
        mapping(address registry => uint256[] troveIds) troveIds;
        mapping(address registry => mapping(uint256 index => uint256 troveId)) idsByIndex;
        mapping(address registry => mapping(uint256 troveId => uint256 index)) indexes;
    }
