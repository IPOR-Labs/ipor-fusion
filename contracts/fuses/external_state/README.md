...
and operate a complete ExternalState market — from fuse deployment through substrate grants,
...
│          ▼                       ▼                        ▼                   │
│  ┌────────────────────────────┐ ┌────────────┐ ┌───────────────────────────┐  │
│  │ ExternalStateOperationFuse │ │ Grants     │ │ ExternalStatePausePreHook │  │
│  │ ExternalStateBalanceFuse   │ │ substrates │ │ (per selector)            │  │
│  │ ExternalStateUnpauseFuse   │ │ via Plasma │ │                           │  │
│  │ ExternalStateRescueFuse    │ │ Vault Gov. │ │                           │  │
│  └─────┬──────────────────────┘ └────────────┘ └────────┬──────────────────┘  │
│        │ delegatecall                                   │                     │
│        │                                                │                     │
│        │ reads/writes ERC-7201 ExternalStateStorage ◀───┘                     │
│        │ at slot 0x17810238...8a00                                            │
└────────┼──────────────────────────────────────────────────────────────────────┘
         │ calls (normal external)
         ▼
┌──────────────────────────────────────────┐
│ ExternalStateExecutor (per vault+market) │
│ ---------------------------------------- │
│ • holds asset tokens                     │          ┌────────────────┐
│ • tracks balances[ba]                    │  actions │ External       │
...
- **Single executor per `(vault, marketId)`.** First `enter` / `createExecutor` call lazy-deploys
  `ExternalStateExecutor(address(this), marketId)`; subsequent calls return the cached address. Attempting
  to use the same vault slot for a second market reverts `ExternalStateMultipleMarketsNotSupported(existing, requested)`.
- **Vault storage is ERC-7201 namespaced.** All fuses read/write through `ExternalStateExecutorStorageLib`
  at slot `0x1781023874512ec457c16827ad102f41a5c5ce1cd7ba8aa8fcd2da52541d8a00`. This isolates
  ExternalState state from every other fuse's storage and makes it safe to delegatecall.
...
### 2.1 `ExternalStateOperationFuse`

| Attribute | Value |
|---|---|
| Kind | Stateless fuse (delegatecall from PlasmaVault) |
| Constructor | `(uint256 marketId)` — reverts `ExternalStateZeroMarketId` on 0 |
| Immutables | `VERSION = address(this)`, `MARKET_ID` |
| Entry points | `enter(ExternalStateOperationFuseEnterData)`, `exit(ExternalStateOperationFuseExitData)`, `createExecutor()` |

**Enter semantics:**
1. `_resolveExecutorAndValidate` — revert `ExternalStateEmptyAssetAndActions` if both `amount == 0` and `actions.length == 0`.
...
**Exit semantics:**
1. `_resolveExecutorAndValidate` with `createIfMissing_ = false` — revert `ExternalStateOperationExecutorNotDeployed` if the
...
**Access control:**
- `onlyVault` — `addBalance`, `removeBalance`, `execute`, `withdrawAssetBalance`, `setSubstrates`.
- `onlyCustodian` — `proposeBalance`, `confirmBalance` (linear scan over cached `custodians[]`).
- Public — `syncSubstrates()`, view functions.

### 2.3 `ExternalStateBalanceFuse`

Stateless fuse implementing `IMarketBalanceFuse`. `balanceOf()` is **not view** — it writes to
`ExternalStateExecutorStorageLib` to update `lastTotalBalance` and `lastCheckedCustodianTimestamp`, and
...
1. Executor deployed (`ExternalStatePreHookExecutorNotDeployed`).
2. Pause flag clear (`ExternalStatePreHookPaused`).
3. Inline big-change check (`ExternalStatePreHookBigChangeDetected`) — closes the window between
   `confirmBalance` and the next `balanceOf()`.
4. Staleness gate (`ExternalStatePreHookStale`) — aggregate staleness based on oldest balance account update.

### 2.5 `ExternalStateUnpauseFuse`
...
`ExternalStateZeroAddress` if `asset == address(0)`.

### 2.7 Libraries

- `ExternalStateSubstrateLib` — encode / decode / classify / validate substrates. 8-bit type + 248-bit
  payload packed in a single `bytes32`.
- `ExternalStateExecutorStorageLib` — ERC-7201 namespaced vault storage at slot
  `0x1781023874512ec457c16827ad102f41a5c5ce1cd7ba8aa8fcd2da52541d8a00`
  = `keccak256(abi.encode(uint256(keccak256("io.ipor.externalState.Executor")) - 1)) & ~0xff`.
...
|:---:|---|---|---|:---:|---|
| `0` | `UNDEFINED` | — | — | — | Reserved / sentinel. Reverts `ExternalStateUnsupportedSubstrate` on decode. |
...
`ExternalStateDuplicateSingletonSubstrate` on `syncSubstrates()`.

### 3.2 Mandatory substrates

`syncSubstrates()` reverts `ExternalStateMandatorySingletonMissing(uint8 typeCode)` unless the following
are granted:
...
```solidity
// ASSET
bytes32 sub = ExternalStateSubstrateLib.encodeAssetSubstrate(USDC);
// sub = 0x01 00..00 <USDC address in last 20 bytes>

// TARGET — authorize `IExternalStateProtocol.deposit(address,uint256)` on `externalStateProtocol`
bytes4  selector = IExternalStateProtocol.deposit.selector;             // e.g. 0xf45346dc
bytes32 sub = ExternalStateSubstrateLib.encodeTargetSubstrate(address(externalStateProtocol), selector);
// sub = 0x02 00..<selector 4 bytes at offset 20..24><externalStateProtocol address in last 20 bytes>

// STALENESS_MAX = 24h
bytes32 sub = ExternalStateSubstrateLib.encodeStalenessMaxSubstrate(24 hours);
...
```solidity
ExternalStateSubstrateType t = ExternalStateSubstrateLib.decodeSubstrateType(sub);
if (t == ExternalStateSubstrateType.ASSET) {
    address asset = ExternalStateSubstrateLib.decodeAddressPayload(sub);
}
if (t == ExternalStateSubstrateType.TARGET) {
    (address target, bytes4 selector) = ExternalStateSubstrateLib.decodeTargetPayload(sub);
}
if (t == ExternalStateSubstrateType.STALENESS_MAX) {
    uint256 seconds_ = ExternalStateSubstrateLib.decodeUint248Payload(sub);
}
```
...
`ExternalStateOperationFuse.enter` / `exit` call `validateAssetGranted`, `validateBalanceAccountGranted`,
and `validateTargetSelectorGranted` per action — each reverts `ExternalStateUnsupportedSubstrate` if the
...
```
Alpha                PlasmaVault         ExternalStateOperationFuse          ExecutorStorageLib       ExternalStateExecutor       External Protocol
  │                       │                           │                               │                         │                         │
  │ execute([ExternalStateEnter])                     │                               │                         │                         │
  │──────────────────────▶│                           │                               │                         │                         │
  │                       │ delegatecall enter(...)   │                               │                         │                         │
  │                       │──────────────────────────▶│                               │                         │                         │
  ...
  │                       │                           │ getOrCreateExecutor           │                         │                         │
  │                       │                           │──────────────────────────────▶│                         │                         │
  │                       │                           │                               │ new ExternalStateExecutor() (constructor)         │
  │                       │                           │                               │────────────────────────▶│                         │
  │                       │                           │                               │ read substrates from vault storage, then          │
  │                       │                           │                               │ executor.setSubstrates → cache populated          │
  │                       │                           │                               │────────────────────────▶│                         │
  │                       │                           │ executor addr                 │                         │                         │
  │                       │                           │◀──────────────────────────────│                         │                         │
  │                       │                           │                               │                         │                         │
...
```
Alpha ──execute([ExternalStateExit])──▶ PlasmaVault
                                  │
                                  │ delegatecall exit(ExternalStateOperationFuseExitData)
                                  ▼
                           ExternalStateOperationFuse
                                  │
                                  │ _resolveExecutorAndValidate (createIfMissing_=false → revert
                                  │  ExternalStateOperationExecutorNotDeployed if missing)
                                  │
                                  ...
                                  │       │
                                  │       ├─ balances[ba] -= valueInUnderlying   (revert ExternalStateExitExceedsTrackedBalance if >)
...
```
Custodian A                    ExternalStateExecutor                    Custodian B
     │                              │                              │
     ...
     │                              │ check: pending.proposer != 0       → else ExternalStateExecutorNoPendingProposal
     │                              │ check: pending.proposer != msg.sender → else ExternalStateExecutorSameProposerAndConfirmer
     │                              │ check: now - proposedAt ≤ stalenessMax → else ExternalStateExecutorProposalExpired
     │                              │ _checkDust()
     │                              │ check: _proposalHash(...) == h      → else ExternalStateExecutorProposalHashMismatch
     │                              │ check: (lastUpdated==0) || (now-lastUpdated ≥ minUpdateInterval)
     │                              │                                    → else ExternalStateExecutorMinUpdateIntervalNotMet
...
new balance @ T0           triggers balance fuse readout               blocked by pre-hook
        ...
        ▼                                     ▼                                          ▼
┌───────────────────────┐  ┌────────────────────────────────────────┐  ┌──────────────────────────────────────┐
│ ExternalStateExecutor │  │ ExternalStateBalanceFuse               │  │ ExternalStatePausePreHook            │
│ confirmBalance()      │  │ balanceOf()                            │  │ run()                                │
│ lastCustodianTS = T0  │  │                                        │  │                                      │
└───────────┬───────────┘  │ (total, bps, ts)                       │  │ if executor == 0:                    │
            │ (no state    │   = executor.getBalanceData()          │  │   revert PreHookExecutorNotDeployed  │
            │  write yet)  │                                        │  │ if paused: revert PreHookPaused      │
            │              │ if ts != lastChecked:                  │  │                                      │
            │              │   |Δ| = |total - prev|                 │  │ if ts != lastChecked:                │
            │              │   if |Δ|/prev > bps:                   │  │   |Δ| = |total - prev|               │
            │              │     setPaused(true)                    │  │   if |Δ|/prev > bps:                 │
            │              │   emit ExternalStateBigChangeDetected  │  │     revert PreHookBigChangeDetected  │
            │              │   setLastChecked(ts)                   │  │ (does NOT write lastChecked)         │
            │              │ setLastTotalBalance(total)             │  │                                      │
            │              │                                        │  │ if now - oldest > stalenessMax:      │
            ...
            ▼
┌──────────────────────┐
│ Atomist signs:       │          unpause(ExternalStateUnpauseData)
│   vault, MARKET_ID,  │──────────────────────────┐
│   confirmedTotal,    │                          ▼
│   nonce, expiration, │              ┌──────────────────────────┐
│   chainid            │              │ ExternalStateUnpauseFuse │
└──────────────────────┘              │ • verifies signature     │
                                      │ • recovers signer        │
                                      │ • hasRole(ATOMIST)       │
                                      │ • confirmedTotal         │
...
```
Alpha ──execute([ExternalStateRescue.rescue(airdroppedToken)])──▶ PlasmaVault
                                                           │
                                                  ...
                                                  ExternalStateRescueFuse
                                                           ...
                                                           │
                                                           │ emit ExternalStateAssetRescued(asset)
```
...
- **`ExternalStateBalanceFuse.balanceOf()` return** — USD WAD (18 decimals), suitable for ERC-4626 NAV
  aggregation.
...
- `ExternalStateRescueFuse.rescue(asset)` sweeps them back to the vault.
...
for each asset in cached assets[]:
    ...
    if balance > allowed: revert ExternalStateExecutorDustCheckFailed(asset, balance, allowed)
```
...
```
  ...
  │              │   (confirm reverts              │        │
  │              │    ExternalStateExecutorProposalExpired,  │        │
...
Edge cases:
- **Same proposer confirms** → `ExternalStateExecutorSameProposerAndConfirmer(msg.sender)`.
- **Hash mismatch** (propose was overwritten between propose and confirm) →
  `ExternalStateExecutorProposalHashMismatch(expected, given)`. The old hash becomes invalid — off-chain
...
- **Rate-limit** (per-account) — `block.timestamp - lastUpdated[ba] < minUpdateInterval` and
  `lastUpdated[ba] != 0` → `ExternalStateExecutorMinUpdateIntervalNotMet`. First-ever update for an
...
`ExternalStateBalanceFuse.balanceOf()` compares current total to previously observed `lastTotalBalance`,
...
if lastCustodianTs != lastChecked:
    if prevTotal != 0 and delta * 10_000 / prevTotal > bigChangeBps:
        setPaused(true)
        emit ExternalStateBigChangeDetected(prevTotal, totalBalance, bigChangeBps)
...
1. User attempts `deposit` → pre-hook computes delta > bps → reverts `ExternalStatePreHookBigChangeDetected`.
2. Keeper / alpha call flows through `_updateMarketsBalances` → balance fuse sets pause flag.
3. User attempts `deposit` again → pre-hook reverts `ExternalStatePreHookPaused` instead.
...
signedMessage = keccak256(abi.encodePacked(
    ...
    nonce,                 // uint256 — atomist-chosen, tracked in ExternalStateStorage.usedUnpauseNonces
    expirationTime,        // uint256 — seconds since epoch
    block.chainid          // uint256
))
```

Signs it with an atomist EOA, then Alpha calls `PlasmaVault.execute([ExternalStateUnpauseFuse.unpause(data)])`
...
revert `ExternalStateUnpauseSignatureExpired`.
...
user calls deposit/mint/withdraw/redeem
  ...
  PlasmaVault._runPreHooks(selector)
           │
           ▼
  ExternalStatePausePreHook.run(selector)
           │
           ├─ executor = ExternalStateExecutorStorageLib.getExecutor()
           ├─ if executor == 0: revert ExternalStatePreHookExecutorNotDeployed
           │
           ├─ if ExternalStateExecutorStorageLib.getPaused(): revert ExternalStatePreHookPaused
           │
           ...
           │         revert ExternalStatePreHookBigChangeDetected
           │
           ├─ oldest = executor.getOldestUpdateTimestamp()
           ├─ if oldest != 0:
           │     max = executor.stalenessMax()
           │     if block.timestamp - oldest > max: revert ExternalStatePreHookStale(oldest, now, max)
           │
...
Below is the complete, ordered sequence to stand up a new ExternalState market from zero. Every step maps
...
- The dedicated `MARKET_ID = IporFusionMarkets.EXTERNAL_STATE` (`= 49`). Defined in
  `contracts/libraries/IporFusionMarkets.sol`. One vault hosts at most one ExternalState market via this
  fuse family — `ExternalStateExecutorStorageLib.getOrCreateExecutor` reverts with
  `ExternalStateMultipleMarketsNotSupported` if a second market id is later attempted.
...
```solidity
ExternalStateOperationFuse opFuse    = new ExternalStateOperationFuse(MARKET_ID);
ExternalStateBalanceFuse   balanceFuse = new ExternalStateBalanceFuse(MARKET_ID);
ExternalStateUnpauseFuse   unpauseFuse = new ExternalStateUnpauseFuse(MARKET_ID);
ExternalStateRescueFuse    rescueFuse  = new ExternalStateRescueFuse(MARKET_ID);
ExternalStatePausePreHook  preHook     = new ExternalStatePausePreHook(MARKET_ID);
```
...
uint256 i;
// --- Assets ---
subs[i++] = ExternalStateSubstrateLib.encodeAssetSubstrate(USDC);
subs[i++] = ExternalStateSubstrateLib.encodeAssetSubstrate(USDT);
// --- Balance accounts (logical buckets; can match custodian organisation) ---
subs[i++] = ExternalStateSubstrateLib.encodeBalanceAccountSubstrate(bucketBrokerA);
subs[i++] = ExternalStateSubstrateLib.encodeBalanceAccountSubstrate(bucketBrokerB);
// --- Custodians (≥2, MUST be distinct) ---
subs[i++] = ExternalStateSubstrateLib.encodeCustodianSubstrate(custodianA);
subs[i++] = ExternalStateSubstrateLib.encodeCustodianSubstrate(custodianB);
// --- Targets (action whitelist) ---
subs[i++] = ExternalStateSubstrateLib.encodeTargetSubstrate(address(externalStateProtocol), IExternalStateProtocol.deposit.selector);
subs[i++] = ExternalStateSubstrateLib.encodeTargetSubstrate(address(externalStateProtocol), IExternalStateProtocol.withdraw.selector);
subs[i++] = ExternalStateSubstrateLib.encodeTargetSubstrate(USDC, IERC20.approve.selector); // if alpha approves USDC
// --- Singletons (mandatory) ---
subs[i++] = ExternalStateSubstrateLib.encodeStalenessMaxSubstrate(24 hours);
subs[i++] = ExternalStateSubstrateLib.encodeBigChangeBpsSubstrate(500);        // 5%
// --- Singletons (recommended) ---
subs[i++] = ExternalStateSubstrateLib.encodeDustThresholdSubstrate(100);       // 1 token per asset
subs[i++] = ExternalStateSubstrateLib.encodeMinUpdateIntervalSubstrate(1 hours);
...
calls[0] = FuseAction({
    fuse: address(opFuse),
    data: abi.encodeCall(ExternalStateOperationFuse.createExecutor, ())
});
PlasmaVault(vault).execute(calls);
```

If any of the following conditions hold after sync, the transaction reverts:
- Duplicate singleton grants → `ExternalStateDuplicateSingletonSubstrate`.
- Missing `STALENESS_MAX` or `BIG_CHANGE_BPS` → `ExternalStateMandatorySingletonMissing(typeCode)`.
- Unknown substrate type (raw byte > 8) → `ExternalStateUnsupportedSubstrate`.

Fix the substrate set and re-try. `ExternalStateExecutor.syncSubstrates()` is public — after fixing
...
4. **Pre-hook smoke:** as a non-role user attempt `deposit(0, user)` → assert revert
   `ExternalStatePreHookExecutorNotDeployed` is NOT emitted (executor exists) and the call passes the
   pre-hook chain.
...
- **Regular NAV updates:** keeper / alpha invokes `PlasmaVault.updateMarketsBalances(marketIds)`
  so `ExternalStateBalanceFuse.balanceOf()` runs and advances `lastCheckedCustodianTimestamp`.
...
- **Unpause after big-change:** atomist signs a payload matching the current confirmed balance;
  alpha submits it via `ExternalStateUnpauseFuse.unpause`.
...
subs = [
    ...
    encodeTargetSubstrate(treasury, IExternalStateTreasury.mintNote.selector),
    encodeTargetSubstrate(treasury, IExternalStateTreasury.redeemNote.selector),
...
### 10.4 Invalid grants (will revert on `syncSubstrates` / bootstrap `setSubstrates`)

| Configuration | Revert |
|---|---|
| Two `STALENESS_MAX` substrates with different values | `ExternalStateDuplicateSingletonSubstrate(5)` |
| No `BIG_CHANGE_BPS` granted | `ExternalStateMandatorySingletonMissing(6)` |
| `encodeStalenessMaxSubstrate(type(uint248).max + 1)` at encode time | `ExternalStateSubstratePayloadOverflow(5, value)` |
| `encodeAssetSubstrate(address(0))` | `ExternalStateZeroAddress` |
| Granted raw `bytes32(uint256(9) << 248)` (type code 9) | `ExternalStateUnsupportedSubstrate(9, raw)` |
...
- **Why accepted:** making the interval global would block legitimate same-day updates across a
  ...
  cadence), a hard operational requirement for ExternalState operators.
...
`ExternalStateExecutor.onlyCustodian` reads the cached `custodians[]` array, populated only by
...
`ExternalStateOperationFuse._convertAmountToUnderlying` and `ExternalStateBalanceFuse.balanceOf` consult
...
`ExternalStateExecutorBalanceAccountStillFunded(ba, residualBalance)` and the cache replacement is
...
- **What is enforced on-chain:** every successful `proposeBalance(BA, _)` or
  ...
  the vault. Reverts with `ExternalStateUnsupportedSubstrate(BALANCE_ACCOUNT, encoded)` otherwise — the
  same selector emitted by fuse-side validation in `ExternalStateOperationFuse._validateSubstratesAndActions`,
...
- **Why vault, not cache:** the executor cache may lag behind the vault substrate set between
  ...
  truth.
- **Off-chain monitoring:** alert on `ExternalStateUnsupportedSubstrate(BALANCE_ACCOUNT, ...)` reverts
...
### `syncSubstrates` / `setSubstrates` — access control & cache stability
...
`setSubstrates(bytes32[])` is the **vault-only push variant** (same validation and cache-rebuild
logic, shared implementation). It exists because the vault's fallback routes every unmatched
selector to the callback handler while `PlasmaVault.execute` is running, so the executor cannot
pull `getMarketSubstrates` during in-execute bootstrap (IL-7895). The fuse code, running in the
vault's delegatecall context, reads the substrates directly from vault storage
(`PlasmaVaultConfigLib.getMarketSubstrates`) and pushes them into the freshly deployed executor.
Because its input is caller-supplied rather than trust-derived from the vault view, it is gated
`onlyVault`.

**Vault-callback audit (IL-7895).** External calls from ExternalState contracts back into the vault, and
whether they can run inside `PlasmaVault.execute` (where the fallback blocks unmatched selectors):

| Caller → vault call | Inside `execute`? | Status |
|---|---|---|
| `ExternalStateExecutor.syncSubstrates` → `getMarketSubstrates` | Never (bootstrap now uses the push path; resync is an external, out-of-execute call) | Safe |
| `ExternalStateExecutor._requireBalanceAccountGrantedOnVault` → `isMarketSubstrateGranted` (`proposeBalance`/`confirmBalance`) | Never (custodian EOAs call the executor directly) | Safe |
| `ExternalStateUnpauseFuse.unpause` → `authority()` | Yes | Safe — `authority()` is on `PlasmaVault`'s own ABI (`AccessManagedUpgradeable`), not behind the fallback |
| `ExternalStateOperationFuse`/`ExternalStateBalanceFuse` → `asset()` | Yes | Safe — `asset()` is on `PlasmaVault`'s own ABI (ERC-4626) |

Rule for future ExternalState changes: any vault getter that lives only on `PlasmaVaultGovernance` (i.e. is
reachable only through the fallback) MUST NOT be called from code paths that can run inside
`PlasmaVault.execute` — read vault storage directly via the config/lib layer instead.
...
Every asset (and the vault underlying) registered on an ExternalState market MUST be priced by an oracle
...
3. Call `ExternalStateOperationFuse.enter` with a small real `asset` amount;
...
5. In a later block, call `ExternalStateOperationFuse.exit`. The pulled underlying amount is computed
...
1. Identify the cause by reading recent `ExternalStateBigChangeDetected` / `ExternalStatePreHookBigChangeDetected`
...
2. Decide whether the reported balance is trustworthy:
   - **Trustworthy** (the real economic change exceeded `bigChangeBps`) → atomist signs an
     unpause payload matching the current balance, alpha submits via `ExternalStateUnpauseFuse.unpause`.
...
4. **Sync.** `executor.syncSubstrates()`.
   - Reverts with `ExternalStateExecutorBalanceAccountStillFunded(oldBA, residualBalance)` if step 1 was
...
`opFuse.exit({balanceAccount: BA, ...})` will revert (the latter with `ExternalStateUnsupportedSubstrate`
...
big-change pause via the `prevTotal == 0` short-circuit in `ExternalStateBalanceFuse.balanceOf`. See
...
| `ExternalStateExecutorDeployed` (on the storage lib) | `executor`, `marketId` | `getOrCreateExecutor` first call |
...
| `ExternalStateOperationFuseEnter` | `version (i)`, `asset (i)`, `amount`, `balanceAccount (i)`, `valueInUnderlying`, `actionsCount` |
| `ExternalStateOperationFuseExit` | `version (i)`, `asset (i)`, `amount`, `balanceAccount (i)`, `valueInUnderlying`, `actionsCount` |

### `ExternalStateBalanceFuse`

| Event | Fields |
|---|---|
| `ExternalStateBigChangeDetected` | `prevTotal`, `newTotal`, `bigChangeBps` |

### `ExternalStateUnpauseFuse`

| Event | Fields |
|---|---|
| `ExternalStateUnpaused` | `signer (i)`, `confirmedTotalBalance`, `nonce` |

### `ExternalStateRescueFuse`

| Event | Fields |
|---|---|
| `ExternalStateAssetRescued` | `asset (i)` |
...
Key custom errors with diagnostic parameters. All declared in `contracts/fuses/external_state/errors/ExternalStateErrors.sol`.

### Substrate

| Error | Meaning |
|---|---|
| `ExternalStateUnsupportedSubstrate(uint8 type, bytes32 encoded)` | Substrate type byte > 8, or grant is missing from `isMarketSubstrateGranted`. |
| `ExternalStateDuplicateSingletonSubstrate(uint8 type)` | More than one of `STALENESS_MAX` / `BIG_CHANGE_BPS` / `DUST_THRESHOLD` / `MIN_UPDATE_INTERVAL` granted. |
| `ExternalStateSubstratePayloadOverflow(uint8 type, uint256 value)` | `uint248` payload exceeded. |
| `ExternalStateMandatorySingletonMissing(uint8 type)` | `STALENESS_MAX` or `BIG_CHANGE_BPS` missing after sync. |
| `ExternalStateZeroAddress()` | Encoder called with `address(0)` / rescue called with zero asset. |

### Operation fuse

| Error | Meaning |
|---|---|
| `ExternalStateZeroMarketId()` | Constructor called with `marketId == 0`. |
| `ExternalStateEmptyAssetAndActions()` | `enter`/`exit` called with both `amount == 0` and `actions.length == 0`. |
| `ExternalStateExitExceedsTrackedBalance(address ba, uint256 requested, uint256 tracked)` | `removeBalance` requested > current tracked. |
| `ExternalStatePriceOracleNotSet()` | `PlasmaVaultLib.getPriceOracleMiddleware() == address(0)`. |
| `ExternalStateInvalidPrice(address asset)` | Oracle returned `price == 0`. |
| `ExternalStateOperationExecutorNotDeployed()` | Exit before any enter / createExecutor. |
| `ExternalStateActionDataTooShort(uint256 actionIndex, uint256 dataLength)` | Action calldata < 4 bytes (cannot extract selector). |
| `ExternalStateMultipleMarketsNotSupported(uint256 existingMarketId, uint256 requestedMarketId)` | Vault slot reused for different market. |

### Executor

| Error | Meaning |
|---|---|
| `ExternalStateExecutorUnauthorizedVault()` | `onlyVault` mismatch on `msg.sender`. |
| `ExternalStateExecutorUnauthorizedCustodian(address caller)` | `onlyCustodian` miss (address not in cached `custodians[]`). |
| `ExternalStateExecutorDustCheckFailed(address asset, uint256 balance, uint256 allowed)` | Asset balance above dust allowance. |
| `ExternalStateExecutorSameProposerAndConfirmer(address custodian)` | Same custodian tried to confirm their own proposal. |
| `ExternalStateExecutorProposalHashMismatch(bytes32 expected, bytes32 given)` | Confirm hash does not match pending. |
| `ExternalStateExecutorProposalExpired(uint256 proposedAt, uint256 now, uint256 ttl)` | `now - proposedAt > stalenessMax`. |
| `ExternalStateExecutorMinUpdateIntervalNotMet(uint256 lastUpdated, uint256 now, uint256 minInterval)` | Confirm too soon after previous confirm. |
| `ExternalStateExecutorNoPendingProposal(address ba)` | Confirm before any propose. |
| `ExternalStateExecutorZeroAddressConstructor()` | Executor constructor called with `vault == address(0)`. |
| `ExternalStateExecutorZeroMarketId()` | Executor constructor called with `marketId == 0`. |
| `ExternalStateExecutorBalanceAccountStillFunded(address ba, uint256 residualBalance)` | `syncSubstrates` cannot purge `ba` because `balances[ba] != 0`. Atomist must drain via `ExternalStateOperationFuse.exit` before revoking the substrate (audit fix 17.1, see §13.8). |

### Pre-hook

| Error | Meaning |
|---|---|
| `ExternalStatePreHookExecutorNotDeployed()` | Pre-hook triggered before executor deployed. |
| `ExternalStatePreHookPaused()` | Pause flag set. |
| `ExternalStatePreHookBigChangeDetected(uint256 prevTotal, uint256 newTotal, uint256 bigChangeBps)` | Inline delta check > bps. |
| `ExternalStatePreHookStale(uint256 oldest, uint256 now, uint256 stalenessMax)` | Oldest balance account exceeded staleness. |

### Unpause fuse

| Error | Meaning |
|---|---|
| `ExternalStateUnpauseNotPaused()` | Called when not paused (or executor not deployed). |
| `ExternalStateUnpauseSignatureExpired()` | `block.timestamp > expirationTime`. |
| `ExternalStateUnpauseSignatureReplay(uint256 nonce)` | Nonce already consumed. |
| `ExternalStateUnpauseSignerNotAtomist(address signer)` | Recovered signer lacks `ATOMIST_ROLE`. |
| `ExternalStateUnpauseBalanceMismatch(uint256 signed, uint256 current)` | `confirmedTotalBalance` ≠ `executor.getBalanceFuseSnapshot().totalBalance`. |

### Rescue fuse

| Error | Meaning |
|---|---|
| `ExternalStateRescueExecutorNotDeployed()` | Rescue before executor exists. |

---

## 16. Testing

- **Unit tests:** `forge test --match-path 'test/unitTest/fuses/external_state/**'`.
- **Fork integration tests:** `forge test --match-path 'test/fuses/external_state/**'` (require valid RPC
  URLs in `.env`).
