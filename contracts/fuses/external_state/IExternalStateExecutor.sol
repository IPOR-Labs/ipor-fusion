...
/// @notice Single action executed by the ExternalState executor.
...
struct ExternalStateExecutorAction {
    address target;
    bytes data;
}

/// @title IExternalStateExecutor
/// @notice Interface of the per-vault ExternalStateExecutor contract. Mirrors the public surface used by
...
/// @author IPOR Labs
interface IExternalStateExecutor {
    // ============================================================
    ...
    ///      `ExternalStateOperationFuse._validateSubstratesAndActions` validates BA against vault substrates
    ...
    ///      `ExternalStateExecutorBalanceAccountStillFunded` error in `errors/ExternalStateErrors.sol`.
    ...
    function execute(ExternalStateExecutorAction[] calldata actions_) external;
    ...
    /// @dev Reverts with `ExternalStateUnsupportedSubstrate(BALANCE_ACCOUNT, _)` if `balanceAccount_` is
    ...
    ///
    ///      **Mandatory singletons revert.** Reverts with `ExternalStateMandatorySingletonMissing` if either
    ...
    ///      `ExternalStateExecutorBalanceAccountStillFunded(ba, residualBalance)`. Atomists MUST execute a
    ///      full `ExternalStateOperationFuse.exit(...)` driving `balances[ba] = 0` before revoking the
    ...
    /// @notice Replace the cached substrates with a set pushed by the vault.
    /// @dev **Access control: vault only** (`ExternalStateExecutorUnauthorizedVault` otherwise). Unlike
    ///      `syncSubstrates`, the input here is caller-supplied rather than read from the vault's
    ///      governance view, so it must be restricted to the vault itself.
    ///
    ///      **Why it exists:** while `PlasmaVault.execute` is running, the vault's fallback routes
    ///      every unmatched selector into the callback handler, so the executor cannot pull
    ///      `getMarketSubstrates` from the vault (reverts `HandlerNotFound`). During in-execute
    ///      bootstrap the fuse code — running in the vault's delegatecall context — reads the
    ///      substrates directly from vault storage and pushes them here instead.
    ///
    ///      Validation, cache-rebuild semantics, orphan purging, and emitted events are identical
    ///      to `syncSubstrates` (shared implementation).
    /// @param substrates_ Encoded substrate set, as stored in the vault for `MARKET_ID`.
    function setSubstrates(bytes32[] calldata substrates_) external;
