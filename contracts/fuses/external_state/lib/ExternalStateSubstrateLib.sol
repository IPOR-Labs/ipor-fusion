...
/// @notice Type discriminator for ExternalState substrates. Encoded in the most significant byte
...
enum ExternalStateSubstrateType {
    UNDEFINED, // 0 - invalid
...
/// @title ExternalStateSubstrateLib
/// @notice Library for encoding, decoding, and validating ExternalState substrates as bytes32 values.
...
/// @author IPOR Labs
library ExternalStateSubstrateLib {
    ...
    function encodeAssetSubstrate(address asset_) internal pure returns (bytes32 encoded) {
        ...
        encoded = bytes32((uint256(ExternalStateSubstrateType.ASSET) << _TYPE_SHIFT) | uint256(uint160(asset_)));
    }
    ...
    /// @return substrateType The decoded `ExternalStateSubstrateType` enum value.
    function decodeSubstrateType(bytes32 substrate_) internal pure returns (ExternalStateSubstrateType substrateType) {
        ...
        if (raw > uint8(ExternalStateSubstrateType.MIN_UPDATE_INTERVAL)) {
            revert ExternalStateErrors.ExternalStateUnsupportedSubstrate(raw, substrate_);
        }
        substrateType = ExternalStateSubstrateType(raw);
    }
    ...
    function isAssetSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.ASSET;
    }

    function isTargetSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.TARGET;
    }

    function isCustodianSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.CUSTODIAN;
    }

    function isBalanceAccountSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.BALANCE_ACCOUNT;
    }

    function isStalenessMaxSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.STALENESS_MAX;
    }

    function isBigChangeBpsSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.BIG_CHANGE_BPS;
    }

    function isDustThresholdSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.DUST_THRESHOLD;
    }

    function isMinUpdateIntervalSubstrate(bytes32 substrate_) internal pure returns (bool) {
        return decodeSubstrateType(substrate_) == ExternalStateSubstrateType.MIN_UPDATE_INTERVAL;
    }
