...
/// @title ExternalStateErrors
/// @notice Centralized custom errors for the ExternalState fuse family (operation, balance, pre-hook, executor, unpause, rescue).
/// @dev All errors carry diagnostic parameters to aid debugging and off-chain monitoring.
/// @author IPOR Labs
library ExternalStateErrors {
    // ============================================================
    // Substrate errors
    // ============================================================
    ...
    error ExternalStateUnsupportedSubstrate(uint8 substrateType, bytes32 encoded);
    ...
    error ExternalStateDuplicateSingletonSubstrate(uint8 substrateType);
    ...
    error ExternalStateMandatorySingletonMissing(uint8 substrateType);
    ...
    error ExternalStateSubstratePayloadOverflow(uint8 substrateType, uint256 value);
    ...
    error ExternalStateActionDataTooShort(uint256 actionIndex, uint256 dataLength);
    ...
    error ExternalStateMultipleMarketsNotSupported(uint256 existingMarketId, uint256 requestedMarketId);

    /// @notice Thrown when the operation fuse exit is invoked before the executor was deployed.
    error ExternalStateOperationExecutorNotDeployed();
    ...
    error ExternalStateExecutorUnauthorizedVault();
    ...
    error ExternalStateExecutorUnauthorizedCustodian(address caller);
    ...
    error ExternalStateExecutorDustCheckFailed(address asset, uint256 balance, uint256 allowed);
    ...
    error ExternalStateExecutorSameProposerAndConfirmer(address custodian);
    ...
    error ExternalStateExecutorProposalHashMismatch(bytes32 expected, bytes32 given);
    ...
    error ExternalStateExecutorProposalExpired(uint256 proposedAt, uint256 now_, uint256 ttl);
    ...
    error ExternalStateExecutorMinUpdateIntervalNotMet(uint256 lastUpdatedAt, uint256 now_, uint256 minInterval);
    ...
    error ExternalStateExecutorNoPendingProposal(address balanceAccount);

    /// @notice Thrown when the executor constructor receives a zero vault address.
    error ExternalStateExecutorZeroAddressConstructor();

    /// @notice Thrown when the executor constructor receives a zero marketId.
    error ExternalStateExecutorZeroMarketId();
    ...
    error ExternalStateExecutorBalanceAccountStillFunded(address balanceAccount, uint256 residualBalance);
    ...
    error ExternalStateExecutorBalanceAccountNotInCache(address balanceAccount);
    ...
    error ExternalStatePreHookPaused();
    ...
    error ExternalStatePreHookStale(uint256 oldestUpdate, uint256 now_, uint256 stalenessMax);
    ...
    error ExternalStatePreHookBigChangeDetected(uint256 previousTotal, uint256 currentTotal, uint256 thresholdBps);

    /// @notice Thrown when the pre-hook runs and the executor has not yet been deployed.
    error ExternalStatePreHookExecutorNotDeployed();
    ...
    error ExternalStateUnpauseSignerNotAtomist(address signer);
    ...
    error ExternalStateRescueExecutorNotDeployed();
    ...
    error ExternalStateRescueOfTrackedAssetForbidden(address asset);
    ...
    error ExternalStateDuplicateBalanceAccountSubstrate(address balanceAccount);
}
