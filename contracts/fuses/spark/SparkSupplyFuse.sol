...
struct SparkSupplyFuseEnterData {
    // amount od DAI to supply
    uint256 amount;
}

struct SparkSupplyFuseExitData {
    // amount of DAI to withdraw
    uint256 amount;
}
...
contract SparkSupplyFuse is IFuse {
    ...
    function enter(bytes calldata data) external {
        SparkSupplyFuseEnterData memory data = abi.decode(data, (SparkSupplyFuseEnterData));
        _enter(data);
    }
    ...
    function exit(bytes calldata data) external {
        SparkSupplyFuseExitData memory data = abi.decode(data, (SparkSupplyFuseExitData));
        _exit(data);
    }
