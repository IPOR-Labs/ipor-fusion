...
/// @notice Enum to identify substrate type in Odos market
/// @dev Encoded as first byte (uint8) of bytes32 substrate
enum OdosSubstrateType {
    Unknown, // 0 - Invalid/unknown type
    Token, // 1 - Token address substrate
    Slippage // 2 - Slippage configuration substrate
}

/// @title OdosSubstrateLib
/// @notice Library for encoding and decoding Odos substrate information to/from bytes32
/// @dev Substrates are used to configure allowed tokens and slippage limits for Odos swaps
...
library OdosSubstrateLib {
    ...
    function encodeTokenSubstrate(address token_) internal pure returns (bytes32 encoded) {
        ...
        encoded = bytes32(uint256(OdosSubstrateType.Token) << 248) | bytes32(uint256(uint160(token_)));
    }
    ...
    function decodeSubstrateType(bytes32 substrate_) internal pure returns (OdosSubstrateType substrateType) {
        // Extract the most significant byte
        substrateType = OdosSubstrateType(uint8(uint256(substrate_) >> 248));
    }
    ...
    function isTokenSubstrate(bytes32 substrate_) internal pure returns (bool isToken) {
        isToken = decodeSubstrateType(substrate_) == OdosSubstrateType.Token;
    }
    ...
    function isSlippageSubstrate(bytes32 substrate_) internal pure returns (bool isSlippage) {
        isSlippage = decodeSubstrateType(substrate_) == OdosSubstrateType.Slippage;
    }
}
