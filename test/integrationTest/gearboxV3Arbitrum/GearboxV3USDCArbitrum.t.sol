...
import "../../../contracts/fuses/erc4626/Erc4626SupplyFuse.sol";
import "../../../contracts/fuses/erc4626/Erc4626BalanceFuse.sol";
...
contract GearboxV3USDCArbitrum is SupplyTest {
    ...
    function setupFuses() public override {
        Erc4626SupplyFuse fuse = new Erc4626SupplyFuse(MARKET_ID);
        fuses = new address[](1);
        fuses[0] = address(fuse);
    }

    function setupBalanceFuses() public override returns (MarketBalanceFuseConfig[] memory balanceFuses) {
        ERC4626BalanceFuse gearboxV3Balances = new ERC4626BalanceFuse(MARKET_ID, priceOracle);

        balanceFuses = new MarketBalanceFuseConfig[](1);
        balanceFuses[0] = MarketBalanceFuseConfig(MARKET_ID, address(gearboxV3Balances));
    }
    ...
    function getExitFuseData(
        ...
        Erc4626SupplyFuseExitData memory enterData = Erc4626SupplyFuseExitData({vault: GEARBOX_V3_POOL, amount: amount_});
        data = abi.encode(enterData);
    }
}
