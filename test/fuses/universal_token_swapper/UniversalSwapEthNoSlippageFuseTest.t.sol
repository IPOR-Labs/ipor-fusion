...
contract UniversalSwapEthNoSlippageFuseTest is Test {
    ...
    UniversalTokenSwapperEthFuseNoSlippage private _universalTokenSwapperFuse;
    ...
    function setUp() public {
        ...
        _withdrawManager = _createWithdrawManager();
    ...
    function testShouldSwapWhenOneHop() external {
        ...
        UniversalTokenSwapperEthEnterDataNoSlippage memory enterData = UniversalTokenSwapperEthEnterDataNoSlippage({
            tokenIn: USDC,
            tokenOut: USDT,
            amountIn: depositAmount,
            data: UniversalTokenSwapperEthDataNoSlippage({
                targets: targets,
                callDatas: callDatas,
    ...
    /// @notice Demonstrates that NoSlippage fuse does NOT enforce any slippage protection.
    /// A swap with extremely bad rate (receiving only 1 wei of tokenOut) succeeds,
    /// whereas the regular UniversalTokenSwapperEthFuse would revert with slippage error.
    function testShouldNotRevertOnBadSlippage_NoSlippageProtection() external {
        // given
        address userOne = address(0x1222);
        uint256 depositAmount = 1_000e6;
        ...
        // Simulate a swap that gives a terrible rate by:
        // 1. Transfer USDC out to the executor (it will be "spent")
        // 2. Separately deal 1 wei of USDT to the executor so it transfers it back (simulating terrible output)
        // We use a mock approach: transfer USDC to executor, then executor returns only dust USDT
        ...
        // when - this succeeds because there is NO slippage check
        PlasmaVault(_plasmaVault).execute(enterCalls);

        // then - swap succeeded (no slippage revert), tokens were exchanged
        uint256 plasmaVaultUsdcBalanceAfter = ERC20(USDC).balanceOf(_plasmaVault);
        uint256 plasmaVaultUsdtBalanceAfter = ERC20(USDT).balanceOf(_plasmaVault);

        assertEq(plasmaVaultUsdcBalanceBefore, depositAmount, "plasmaVaultUsdcBalanceBefore");
        assertEq(plasmaVaultUsdcBalanceAfter, 0, "plasmaVaultUsdcBalanceAfter should be 0");
        assertEq(plasmaVaultUsdtBalanceBefore, 0, "plasmaVaultUsdtBalanceBefore");
        // The key assertion: swap completed without slippage revert, any amount of output is accepted
        assertGt(plasmaVaultUsdtBalanceAfter, 0, "plasmaVaultUsdtBalanceAfter should be > 0");
    }
    ...
    function _setupMarketConfigs() private returns (MarketSubstratesConfig[] memory marketConfigs_) {
        ...
        // NoSlippage fuse uses isSubstrateAsAssetGranted which expects plain address-encoded substrates
        bytes32[] memory universalSwapSubstrates = new bytes32[](16);
        universalSwapSubstrates[0] = bytes32(uint256(uint160(USDC)));
        universalSwapSubstrates[1] = bytes32(uint256(uint160(USDT)));
        universalSwapSubstrates[2] = bytes32(uint256(uint160(DAI)));
        universalSwapSubstrates[3] = bytes32(uint256(uint160(STETH)));
        universalSwapSubstrates[4] = bytes32(uint256(uint160(W_ETH)));
        universalSwapSubstrates[5] = bytes32(uint256(uint160(ETHX)));
        universalSwapSubstrates[6] = bytes32(uint256(uint160(FRXETH)));
        universalSwapSubstrates[7] = bytes32(uint256(uint160(SFRXETH)));
        universalSwapSubstrates[8] = bytes32(uint256(uint160(RETH)));
        universalSwapSubstrates[9] = bytes32(uint256(uint160(_UNIVERSAL_ROUTER)));
        universalSwapSubstrates[10] = bytes32(uint256(uint160(STADER_STAKING_POOL_MANAGER)));
        universalSwapSubstrates[11] = bytes32(uint256(uint160(FRAX_ETHER_MINTER_V2_ADDRESS)));
        universalSwapSubstrates[12] = bytes32(uint256(uint160(ROCKET_DEPOSIT_POOL_ADDRESS)));
        universalSwapSubstrates[13] = bytes32(0);
        universalSwapSubstrates[14] = bytes32(0);
        universalSwapSubstrates[15] = bytes32(0);
    ...
    function _setupFuses() private returns (address[] memory fuses_) {
        ...
        _universalTokenSwapperFuse = new UniversalTokenSwapperEthFuseNoSlippage(
