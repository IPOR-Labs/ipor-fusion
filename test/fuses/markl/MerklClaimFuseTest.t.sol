...
contract MerklClaimFuseTest is Test {
    ...
    function testClaimRewards() public {
        ...
        uint256 usdcBalanceBefore = IERC20(USDC).balanceOf(REWARDS_CLAIM_MANAGER);
        uint256 morphoBalanceBefore = IERC20(MORPHO).balanceOf(REWARDS_CLAIM_MANAGER);
        uint256 rEULBalanceBefore = IERC20(rEUL).balanceOf(PLASMA_VAULT);
        ...
        // then
        uint256 usdcBalanceAfter = IERC20(USDC).balanceOf(REWARDS_CLAIM_MANAGER);
        uint256 morphoBalanceAfter = IERC20(MORPHO).balanceOf(REWARDS_CLAIM_MANAGER);
        uint256 rEULBalanceAfter = IERC20(rEUL).balanceOf(PLASMA_VAULT);

        assertEq(usdcBalanceBefore, 0, "USDC balance should be 0 before claim");
        assertEq(morphoBalanceBefore, 0, "MORPHO balance should be 0 before claim");
        assertEq(rEULBalanceBefore, 0, "rEUL balance should be 0 before claim");

        // Verify that rewards were claimed successfully
        assertEq(usdcBalanceAfter, amounts[0], "USDC rewards should be claimed");
        assertEq(morphoBalanceAfter, amounts[1], "MORPHO rewards should be claimed");
        assertEq(rEULBalanceAfter, amounts[2], "rEUL rewards should be claimed");
    }
}
