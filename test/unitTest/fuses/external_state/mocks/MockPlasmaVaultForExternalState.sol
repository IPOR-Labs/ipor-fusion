...
/// @title MockPlasmaVaultForExternalState
/// @notice Minimal Plasma-Vault-like harness used by ExternalState unit tests.
...
///      the `IPlasmaVaultGovernance` methods actually consumed by the ExternalState fuses and the
///      `IERC4626.asset()` method required by the operation and balance fuses.
contract MockPlasmaVaultForExternalState {
    ...
    // -------------------------------------------------
    // IERC4626 (only the pieces ExternalState fuses call)
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // IPlasmaVaultGovernance (only the pieces ExternalState fuses call)
    // -------------------------------------------------
    ...
    /// @notice Mirrors PlasmaVault's IAccessManaged.authority(), used by ExternalStateUnpauseFuse.
